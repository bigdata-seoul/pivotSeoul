...
import type {
  ...
  /**
   * 공통 fetch 래퍼
   * - JSON 파싱 및 에러 핸들링을 공통 처리
   * - 4xx / 5xx 응답은 Error로 throw
   */
   ...
   * DELETE /api/scenarios/{scenarioId}
   */
  export const deleteScenario = (scenarioId: number): Promise<void> =>
    request(`/api/scenarios/${scenarioId}`, { method: 'DELETE' });
   ...
   * POST /api/scenarios/{scenarioId}/variables/bulk
   * TODO: 프론트 변수 일괄 저장 구현 시 컴포넌트에서 연동
   */
